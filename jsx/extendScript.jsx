...
$._PPP_ = {
	//When change
	  //Get CTI
	  //Get markers
	  //See if markers have changed, if multiple then record it
	  //See how much
	  //Make that change to all markers after CTI
  ...
  setWorkspace: function() {
    ...
    if (workspaces) {
      ...
      for (var i = 0;
        ((i < workspaces.length) && (foundIt === false)); i++) {
  ...
  removePiece: function() {
    ...
    var actS = app.project.activeSequence;
    var actN = actS.name;
    var seqInPoint = actS.getInPoint(); // new in 9.0
    var seqOutPoint = actS.getOutPoint(); // new in 9.0
    var seqInPointAsTime = actS.getInPointAsTime(); // new in 12.0
    var seqOutPointAsTime = actS.getOutPointAsTime(); // new in 12.0


    // set player position, parameter is time in seconds
    $._PPP_.setPlayerPosition(seqOutPoint);
    //app.setSDKEventMessage(convertSec, 'info'); //updates Event Message (message, type)
    // var time = activeSequence.CTI.timecode; // CTI = Current Time Indicator.
    // var timeSecs	= activeSequence.CTI.secs; // Just for reference, here's how to access the CTI
    // var timeFrames	= activeSequence.CTI.frames; // (Current Time Indicator), for the active sequence.
    // var timeTicks	= activeSequence.CTI.ticks;
  ...
  setPlayerPosition: function(pos) {
    var actS = app.project.activeSequence;
    var timeB = actS.timebase; //number of ticks in a frame
    var projectItemNum = $.runScript.getActiveFrameRate();
    var convertSec = Math.round(projectItemNum[0] * pos);
    actS.setPlayerPosition(convertSec * timeB);
  },
  findItem: function(name) {
    var curI = app.project.rootItem;
    var num = curI.children.numItems
    for (i = 0; i < num; i++) {
      var curBin = curI.children[i];
      var curName = curBin.name;
      if (curName == name) {
        var fr = curBin.getFootageInterpretation();
        var fr2 = fr.frameRate;
        return fr2
      }

    }
  },
  getActiveFrameRate: function() {
    try {
      var activeSequence = app.project.activeSequence;
      if (activeSequence) {
        var allSettings = activeSequence.getSettings();
        var frameRateCode = allSettings.videoDisplayFormat;
        /*
		                 Frame rate codes. Non drop is what you see, drop is one up. so 59.94 in drop would be 60.
        ...
        if (frameRateNum == null) {
          throw new Error("Please make sure your time display is set correctly");
        } else {
          return [frameRateNum, dropYes];
        }

      } else {

        throw new Error("Please make sure your sequence is open and active then try again");
      }
    } catch (err) {
      alert("Error trying to get current sequences frame rate " + "\n" + err.line + "\n" + err.toString());
    }
  }
